...
int main(int argc, char* argv[])
{
	...
	PyObject* cur = PyImport_ImportModule("bmake");

	if (!cur)
	{
		logger(LogLevel::Error) << "bmake.py does not exist in the current directory";
		return -1;
	}
	PyObject* method = PyObject_GetAttrString(cur, argv[1]);
	if (!method)
	{
		logger(LogLevel::Error) << "work Method:" << argv[1] << " does not exist in bmake.py";
		return -1;
	}
